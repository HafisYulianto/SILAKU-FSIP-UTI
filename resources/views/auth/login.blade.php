<x-layouts.guest :title="'Login'">
    <div class="relative">
        ...
        <div class="absolute -top-16 -left-16 w-48 h-48 bg-emerald-400/20 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute -bottom-16 -right-16 w-48 h-48 bg-teal-400/20 rounded-full blur-3xl pointer-events-none"></div>

        <!-- Glassmorphic Login Card -->
        <div class="relative bg-emerald-950/65 backdrop-blur-2xl rounded-3xl p-8 sm:p-10 shadow-2xl border border-white/15 transition-all duration-500 overflow-hidden">
            <!-- Glass gloss line decoration -->
            <div class="absolute -top-[100%] left-0 w-[50%] h-[300%] bg-gradient-to-r from-transparent via-white/5 to-transparent rotate-45 pointer-events-none"></div>

            <!-- Brand logo & header -->
            <div class="text-center mb-8 relative z-10">
                <div class="inline-flex w-20 h-20 bg-white/10 border border-white/20 rounded-2xl items-center justify-center shadow-2xl mb-4 backdrop-blur-md">
                    <img src="{{ asset('images/002-UTI.png') }}" alt="Logo UTI" class="h-14 w-auto object-contain drop-shadow-lg">
                </div>
                <h2 class="text-3xl font-black text-white tracking-tight leading-none">SILAKU</h2>
                <p class="text-xs text-emerald-300 font-semibold tracking-wider uppercase mt-2.5">Sistem Pelaporan IKU</p>
            ...
            <form method="POST" action="{{ route('login.submit') }}" id="login-form" class="space-y-6 relative z-10">
                ...
                {{-- Email --}}
                <div>
                    <label for="email" class="block text-sm font-semibold text-emerald-100 mb-2">Email</label>
                    <div class="relative">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus
                               class="w-full bg-white/5 hover:bg-white/10 focus:bg-emerald-950/80 border border-white/10 focus:border-emerald-400 rounded-xl pl-12 pr-4 py-3.5 text-white placeholder-emerald-100/20 focus:outline-none focus:ring-2 focus:ring-emerald-400/20 transition-all duration-300"
                ...
                <div>
                    ...
                    <div x-data="{ show: false }" class="relative">
                        ...
                        <input :type="show ? 'text' : 'password'" name="password" id="password" required
                               class="w-full bg-white/5 hover:bg-white/10 focus:bg-emerald-950/80 border border-white/10 focus:border-emerald-400 rounded-xl pl-12 pr-12 py-3.5 text-white placeholder-emerald-100/20 focus:outline-none focus:ring-2 focus:ring-emerald-400/20 transition-all duration-300"
                               placeholder="••••••••">
